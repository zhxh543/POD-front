<template>
  <div id="appUserEmpower">
    <div  class="el_content">
      ...
      <template>
        <!--审核管理-->
        <div  v-for="(list,index) in roleList" :key="index" v-loading="loading">
          <el-checkbox :indeterminate="list.isIndeterminate" v-model="list.isCheckAll"  @change="handleCheckAll(list,index)">{{list.showName}}</el-checkbox>
          <div class="checkbox-group">
            <el-checkbox-group v-model="selectData" @change="handleChange()">
              <el-checkbox v-for="(item,indx) in list.children" :label="item.id" :key="item.id" border @change="appendParentId(list.children,item.parentId,item.id,index,roleList)"><span>{{item.showName}}</span></el-checkbox>
            </el-checkbox-group>
          </div>
        </div>
      </template>
...
<script>
    export default {
      ...
      methods: {
        // 获取权限
        getEmpower () {
          let url = "/bookCategoryPermission/"+this.userId
        ...
        // 获取图书分类列表
        getData (data) {
          let url = "/warning/basic/findTree"
          this.loading = true
        ...
        submitForm() {
          let url = "/bookCategoryPermission/"+this.userId+"/grant"
