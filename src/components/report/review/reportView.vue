...
<script>
  ...
  export default {
    ...
    methods: {
      getSettings(code) {
        if (code) {
          this.$ajax.getMessage('/chartBook/' + code).then(res => {
            if (res.status === 200)
              this.setMain(JSON.parse(res.data.settings));
            else if (res.status === 401) {
              this.$confirm('该页面需要登录后才能查看, 是否登录?', '提示', {
                confirmButtonText: '登录',
                cancelButtonText: '取消',
                type: 'warning'
              }).then(() => {
                window.location.href = `${this.$ajax.getUrl()}/#/login`;
              }).catch(() => {
                return false;
              });
            } else
    ...
    mounted() {
      ...
      this.getSettings(this.$route.query.code);
    }
  }
</script>
