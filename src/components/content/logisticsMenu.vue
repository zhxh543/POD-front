...
<script>
  import inventoryManage from '@/components/logisticsManage/inventoryManage.vue'
  import orderMange from '@/components/logisticsManage/orderMange.vue'
  ...
  export default {
    ...
    methods: {
      ...
      this.current = name;
        this.$router.push({name: "logisticsManage", query: params});
      },
      //监听路由发生变化
      listenRouter() {
        if (this.$route.query.name) {
      ...
      changeComponent(page) {    //点击左侧nav切换组件
        ...
        if(url.indexOf("http") != -1){
          ...
          this.$router.push({name: 'logisticsManage', query: {name: page}});
          this.current = page;
        }
      },
      hrefUrl (url){
