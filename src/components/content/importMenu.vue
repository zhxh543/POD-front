...
<script>
  import uncoilImportManage from '@/components/importData/uncoilImportManage.vue'
  import internetShopManage from '@/components/importData/internetShopManage.vue'
  import otherImportManage from '@/components/importData/otherImportManage.vue'
  ...
  export default {
    ...
    methods: {
      ...
      changeCurrentView() {    // 刷新页面定位到当前组件
        ...
        this.current = name;
        this.$router.push({name: "importManage", query: params});
      },
      //监听路由发生变化
      listenRouter() {
        if (this.$route.query.name) {
      ...
      changeComponent(page) {    //点击左侧nav切换组件
        ...
        if(url.indexOf("http") != -1){
          ...
          this.$router.push({name: 'importManage', query: {name: page}});
          this.current = page;
        }
      },
      hrefUrl (url){
