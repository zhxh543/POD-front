...
<script>
  import factoryDataMange from '@/components/printingManage/factoryDataMange.vue'
  import costParameterMange from '@/components/printingManage/costParameterMange.vue'
  ...
  export default {
    ...
    methods: {
      ...
      changeCurrentView() {    // 刷新页面定位到当前组件
        ...
        this.current = name;
        this.$router.push({name: "printingManage", query: params});
      },
      //监听路由发生变化
      listenRouter() {
        if (this.$route.query.name) {
      ...
      changeComponent(page) {    //点击左侧nav切换组件
        ...
        if(url.indexOf("http") != -1){
          ...
          this.$router.push({name: 'printingManage', query: {name: page}});
          this.current = page;
        }
      },
      hrefUrl (url){
