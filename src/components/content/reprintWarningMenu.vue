...
<script>
  import reprintWarning from '@/components/reprintWarningManage/reprintWarning.vue'
  import setWarningManage from '@/components/reprintWarningManage/setWarningManage.vue'
  import warningAreaManage from '@/components/reprintWarningManage/warningAreaManage.vue'
  ...
  export default {
    ...
    methods: {
      ...
      this.current = name;
        this.$router.push({name: "reprintWarningManage", query: params});
      },
      //监听路由发生变化
      listenRouter() {
        if (this.$route.query.name) {
      ...
      changeComponent(page) {    //点击左侧nav切换组件
        ...
        if(url.indexOf("http") != -1){
          ...
          this.$router.push({name: 'reprintWarningManage', query: {name: page}});
          this.current = page;
        }
      },
      hrefUrl (url){
