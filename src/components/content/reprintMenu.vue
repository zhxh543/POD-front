...
<script>
  import sendOutManage from '@/components/reprintManage/sendOutManage.vue'
  import receiveManage from '@/components/reprintManage/receiveManage.vue'
  import printSpeedMange from '@/components/reprintManage/printSpeedMange.vue'
  import printSpeedStateMange from '@/components/reprintManage/printSpeedStateMange.vue'
  ...
  export default {
    ...
    methods: {
      ...
      this.current = name;
        this.$router.push({name: "reprintManage", query: params});
      },
      //监听路由发生变化
      listenRouter() {
        if (this.$route.query.name) {
      ...
      changeComponent(page) {    //点击左侧nav切换组件
        ...
        if(url.indexOf("http") != -1){
          ...
          this.$router.push({name: 'reprintManage', query: {name: page}});
          this.current = page;
        }
      },
      hrefUrl (url){
