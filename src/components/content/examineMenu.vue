...
<script>
  import produceManage from '@/components/examineManage/produceManage.vue'
  import reprintManage from '@/components/examineManage/reprintManage.vue'
  ...
  export default {
    ...
    methods: {
      ...
      this.current = name;
        this.$router.push({name: "examineManage", query: params});
      },
      //监听路由发生变化
      listenRouter() {
        if (this.$route.query.name) {
        ...
        if(url.indexOf("http") != -1){
          return
            ...
            this.$router.push({name: 'examineManage', query: {name: page}});
          this.current = page;
        }
      },
      hrefUrl (url){
